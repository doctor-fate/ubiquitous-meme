...
public class JoinReducer extends Reducer<CompositeWritable, FlightAirportWritable, Text, Text> {
    @Override
    protected void reduce(CompositeWritable key, Iterable<FlightAirportWritable> values, Context context)
        ...
        if (n > 0) {
            context.write(new Text(name), new Text(String.format("min=%f;max=%f;avg=%f", min, max, avg / n)));
        }
    }
}